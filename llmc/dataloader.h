...
int64_t dataloader_load_shard_(DataLoader *loader, int shard_index) {
    ...
    // -1 uint32_t due to us taking B*T+1 tokens but moving by B*T tokens
...
void dataloader_next_batch(DataLoader *loader) {
    ...
    size_t B = loader->B;
    size_t T = loader->T;
    // read B*T+1 uint32_t tokens from the file into buffer
...
void evalloader_init(EvalLoader *loader,
    ...
    // up to T tokens, and their tokens are uint32_t (so 2 bytes/token).
